...
// src/helpers.ts
var createVersionText = (type = "Pre-Release", version, context2) => {
  if (!context2.payload.comment) {
    return "";
  }
  return `\u{1F389} ${type} version ` + version + " publish successful! You can install prerelease version via `npm install package@" + version + " ` [@" + context2.payload.comment.user.login + "](https://github.com/" + context2.payload.comment.user.login + ")\n\n```\n" + version + "\n```\n" + process.env.GITHUB_STEP_SUMMARY;
};

// src/utils/context.ts
var getRepo = (context2) => {
  const owner = process.env.OWNER || context2.repo.owner;
  const repo = process.env.REPO || context2.repo.repo;
  return { owner, repo };
};
...
call(async ({ github, context: context2, core: core2 }) => {
  ...
  const repo = getRepo(context2);
  const issueNumber = process.env.ISSUE_NUMBER ? parseInt(process.env.ISSUE_NUMBER) : context2.issue.number;
