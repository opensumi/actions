...
async function call(asyncFunction, callOptions = {}) {
  ...
  const github = (0, import_github2.getOctokit)(token, opts);
...
call(async ({ github, context: context3, core: core2 }) => {
  var _a;
  ...
  const { data: diff } = await octo.pulls.get({
    ...
    pull_number: context3.issue.number,
    mediaType: {
      format: "diff"
    }
  });
  const title = (_a = context3.payload.pull_request) == null ? void 0 : _a.title;
